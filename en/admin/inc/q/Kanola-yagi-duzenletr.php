...
if ($_POST) {
    ...
    if (empty($_FILES["resim1"]['name'])) {
        ...
        $insert = $db->exec("UPDATE  kanolayagitr SET kanolayagi_p=0, kanolayagi_text='{$Konala_text}' WHERE kanolayagiid={$id}");
        ...
        if (move_uploaded_file($_FILES["$posted"]['tmp_name'], "resimler/" . $yeniad)) {
            $insert = $db->exec("UPDATE  kanolayagitr SET  kanolayagi_url='{$hedef1}',kanolayagi_p=0, kanolayagi_text='{$Konala_text}' WHERE kanolayagiid={$id}");
            //$insert->execute();
            echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
            header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=Kanola-yagitr");
        } else {
...
$sor = $db->query("SELECT * FROM kanolayagitr WHERE kanolayagiid={$id}");
$Blog = $sor->fetch(PDO::FETCH_ASSOC);
?>

<section class="section">
    <div class="section-inner">
        <h2 class="heading"><i class="fa fa-plus"></i> Kanola Yağı Düzenle TR<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=Kanola-yagitr"; ?>"><i class="fa fa-file-text-o"></i> Blokları Göster </a></small></h2>
