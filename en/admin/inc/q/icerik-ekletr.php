...
<?php
	if($_POST){
		...
		if(empty($content_title) || empty($content) || empty($content_img)){
			...
			$sorgu = $db->prepare("SELECT COUNT(*) FROM contentstr WHERE content_url='{$content_url}'");
			$sorgu->execute();
			...
			if($say>0){
				...
				$insert = $db->prepare("INSERT INTO contentstr SET content_title='{$content_title}', content_img='{$content_img}', content_comments='{$content_comments}', category_id='{$category_id}', content='{$content}', content_p=0, content_url='{$content_url}'");
				$insert->execute();
...
<section class="section">
	...
	<h2 class="heading"><i class="fa fa-plus"></i> İçerik Ekle TR<br/><small><a href="<?php echo URL."/en/admin/index.php?do=iceriktr"; ?>"><i class="fa fa-file-text-o"></i> İçerikleri Göster</a></small></h2>
	...
	<div class="item row">
			...
			<?php }else { ?>
				<p class="alert alert-danger">Hiç kategori eklemediğiniz için içerik ekleyemiyorsunuz. Hemen şimdi <a href="<?php echo URL."/en/admin/index.php?do=kategori-ekletr"; ?>">Kategori Ekle</a></p>
			<?php } ?>
		</div>
	</div>
</section>