...
<h2 class="heading"><i class="fa fa-file-text-o"></i> Kanola Sayfası Yağı EN<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=kanolayagisayfa-ekleen"; ?>"><i class="fa fa-plus"></i>
...
if ($_POST) {
    ...
    if (empty($_FILES["resim1"]['name'])) {
        ...
        $insert = $db->exec("UPDATE  kanolayagisayfaen SET kanolayagi_text='{$sayfa_text}',kanolayagi_title='{$sayfa_title}' ");
        echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";

        header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=kanolayagisayfaen");
        ...
        if (move_uploaded_file($_FILES["$posted"]['tmp_name'], "resimler/" . $yeniad)) {
            $insert = $db->exec("UPDATE  kanolayagisayfaen SET  kanolayagi_url='{$hedef1}',kanolayagi_title='{$sayfa_title}',kanolayagi_text='{$sayfa_text}' ");
            //$insert->execute();
            echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
            header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=kanolayagisayfaen");
        } else {
...
$setting = $db->query("SELECT * FROM kanolayagisayfaen ")->fetch(PDO::FETCH_ASSOC);
?>
