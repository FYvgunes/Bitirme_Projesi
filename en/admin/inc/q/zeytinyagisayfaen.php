...
<h2 class="heading"><i class="fa fa-file-text-o"></i>Zeytin yağı EN<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=zeytinyagisayfa-ekleen"; ?>"><i class="fa fa-plus"></i>
...
if ($_POST) {
    ...
    if (empty($_FILES["resim1"]['name'])) {
        ...
        $insert = $db->exec("UPDATE  zeytinyagisayfaen SET zeytinyag_text='{$sayfa_text}',zeytinyag_title='{$sayfa_title}' ");
        echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";

        header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=zeytinyagisayfaen");
        ...
        if (move_uploaded_file($_FILES["$posted"]['tmp_name'], "resimler/" . $yeniad)) {
            $insert = $db->exec("UPDATE  zeytinyagisayfaen SET  zeytinyag_url	='{$hedef1}',zeytinyag_title='{$sayfa_title}',zeytinyag_text='{$sayfa_text}' ");
            //$insert->execute();
            echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
            header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=zeytinyagisayfaen");
        } else {
...
$setting = $db->query("SELECT * FROM zeytinyagisayfaen ")->fetch(PDO::FETCH_ASSOC);
?>
