...
if ($_POST) {
    ...
    if (empty($_FILES["resim1"]['name'])) {
        ...
        $insert = $db->exec("UPDATE  kopukbardakde SET kopukbardak_p=0, kopukbardak_title='{$Konala_text}' WHERE kopukbardak_id={$id}");
        ...
        if (move_uploaded_file($_FILES["$posted"]['tmp_name'], "resimler/" . $yeniad)) {
            $insert = $db->exec("UPDATE  kopukbardakde SET  kopukbardak_url='{$hedef1}',kopukbardak_p=0, kopukbardak_title='{$Konala_text}' WHERE kopukbardak_id={$id}");
            //$insert->execute();
            echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
            header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=kopukbardakde");
        } else {
...
$sor = $db->query("SELECT * FROM kopukbardakde WHERE kopukbardak_id={$id}");
$Blog = $sor->fetch(PDO::FETCH_ASSOC);
?>

<section class="section">
    <div class="section-inner">
        <h2 class="heading"><i class="fa fa-plus"></i> Köpük Bardak Düzenle DE<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=kopukbardakde"; ?>"><i class="fa fa-file-text-o"></i> içerikleri Göster </a></small></h2>
