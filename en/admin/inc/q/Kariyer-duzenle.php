...
if ($_POST) {
	...
	if (empty($Kariyer_title) || empty($Kariyer)) {
		...
		$sorgu = $db->prepare("SELECT COUNT(*) FROM kariyer WHERE Kariyer_url='{$Kariyer_url}' AND Kariyer_id != {$id}");
		$sorgu->execute();
		...
		if ($say > 0) {
			...
			$Kariyer_nit1 = p("Kariyer_nit1");
			$Kariyer_nit1 = p("Kariyer_nit1");
			$insert = $db->prepare("UPDATE kariyer SET Kariyer_title='{$Kariyer_title}',  Kariyer='{$Kariyer}',  Kariyer_url='{$Kariyer_url}',Kariyer_nit1='{$Kariyer_nit1}',Kariyer_nit2='{$Kariyer_nit2}',Kariyer_nit3='{$Kariyer_nit3}',Kariyer_nit4='{$Kariyer_nit4}',Kariyer_nit5='{$Kariyer_nit5}' WHERE Kariyer_id={$id}");
			$insert->execute();
