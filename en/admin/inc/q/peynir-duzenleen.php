...
$id = g('id');
if (empty($id)) {
    header("Location: " . URL . "/en/admin/index.php?do=peyniren");

    exit;
} else {
    $sor = $db->query("SELECT * FROM peyniren WHERE peynir_id={$id} AND peynir_p=0");
...
if ($_POST) {
    ...
    if (empty($_FILES["resim1"]['name'])) {
        ...
        $insert = $db->exec("UPDATE  peyniren SET peynir_p=0, peynir_text='{$Konala_text}' WHERE peynir_id={$id}");
        echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
        header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=peyniren");
        ...
        if (move_uploaded_file($_FILES["$posted"]['tmp_name'], "resimler/" . $yeniad)) {
            $insert = $db->exec("UPDATE  peyniren SET  peynir_url='{$hedef1}',peynir_p=0, peynir_text='{$Konala_text}' WHERE peynir_id={$id}");
            //$insert->execute();
            echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
            header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=peyniren");
        } else {
...
$sor = $db->query("SELECT * FROM peyniren WHERE peynir_id={$id}");
$Blog = $sor->fetch(PDO::FETCH_ASSOC);
?>

<section class="section">
    <div class="section-inner">
        <h2 class="heading"><i class="fa fa-plus"></i> Zeytin Yağı Düzenle EN<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=peyniren"; ?>"><i class="fa fa-file-text-o"></i> içerikleri Göster </a></small></h2>
