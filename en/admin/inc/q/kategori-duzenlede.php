...
if ($_POST) {
    ...
    if (empty($category_title)) {
        ...
        if ($say > 0) {
            ...
            $insert = $db->prepare("UPDATE categoriesde SET category_title='{$category_title}', category_url='{$category_url}', category_menu='{$category_menu}', category_onoff=1 WHERE category_id={$id}");
            $insert->execute();
