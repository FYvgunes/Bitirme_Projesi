...
<?php
	$id = g('id');
	if(empty($id) || !is_numeric($id)){ header("Location: ".URL."/en/admin/index.php?do=icerik"); exit; }else{
		$sor = $db->query("SELECT * FROM contents WHERE content_id={$id} AND content_p=0");
		$icerik = $sor->fetch(PDO::FETCH_ASSOC);
		$icerik_varmi = $sor->rowCount();
		if($icerik_varmi>0){ null; }else{
			header("Location: ".URL."/en/admin/index.php?do=icerik"); exit;
		}
	}
	...
	if($_POST){
		...
		if(empty($content_title) || empty($content) || empty($content_img)){
			...
			$sorgu = $db->prepare("SELECT COUNT(*) FROM contents WHERE content_url='{$content_url}' AND content_id != {$id}");
			$sorgu->execute();
			...
			if($say>0){
				...
				$insert = $db->prepare("UPDATE contents SET content_title='{$content_title}', content_img='{$content_img}', content_comments='{$content_comments}', category_id='{$category_id}', content='{$content}', content_p=0, content_url='{$content_url}' WHERE content_id={$id}");
				$insert->execute();
...
<section class="section">
	<div class="section-inner">
		<h2 class="heading"><i class="fa fa-edit"></i> İçerik Düzenle EN<br/><small><a href="<?php echo URL."/en/admin/index.php?do=icerik"; ?>"><i class="fa fa-file-text-o"></i> İçerikleri Göster</a></small></h2>
		<div class="item row">
