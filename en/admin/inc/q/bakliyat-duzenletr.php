...
if ($_POST) {
    ...
    if (empty($_FILES["resim1"]['name'])) {
        ...
        $insert = $db->exec("UPDATE  bakliyattr SET bakliyat_p=0, bakliyat_title='{$Konala_text}' WHERE bakliyat_id={$id}");
        ...
        if (move_uploaded_file($_FILES["$posted"]['tmp_name'], "resimler/" . $yeniad)) {
            $insert = $db->exec("UPDATE  bakliyattr SET  bakliyat_url='{$hedef1}',bakliyat_p=0, bakliyat_title='{$Konala_text}' WHERE bakliyat_id={$id}");
            //$insert->execute();
            echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
            header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=bakliyattr");
        } else {
...
$sor = $db->query("SELECT * FROM bakliyattr WHERE bakliyat_id={$id}");
$Blog = $sor->fetch(PDO::FETCH_ASSOC);
?>

<section class="section">
    <div class="section-inner">
        <h2 class="heading"><i class="fa fa-plus"></i> Bakliyat Düzenle TR<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=bakliyattr"; ?>"><i class="fa fa-file-text-o"></i> içerikleri Göster </a></small></h2>
