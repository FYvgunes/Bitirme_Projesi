...
if ($_POST) {
    ...
    $Konala_text = p('kartonbardak_text');
    $Konala_title = p('kartonbardak_title');
    $posted = "resim1";
    ...
    if ($_FILES["resim1"]['name']) {
        ...
        if (move_uploaded_file($_FILES["$posted"]['tmp_name'], "resimler/" . $yeniad)) {
            $insert = $db->exec("INSERT INTO kartonbardaksayfade SET  kartonbardak_url='{$hedef1}', kartonbardak_title='{$Konala_title}', kartonbardak_text='{$Konala_text}'");
            //$insert->execute();
            echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
            header("Refresh: 0; url=" . URL . "/en/admin/index.php?do=kartonbardaksayfade");
        } else {
...
<section class="section">
    <div class="section-inner">
        ...
        <h2 class="heading"><i class="fa fa-plus"></i> Karton Bardak Sayfa DE<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=kartonbardaksayfade"; ?>"><i class="fa fa-file-text-o"></i> İçerikleri Göster </a></small></h2>

        <div class="item row">
            <form action="" method="POST" enctype="multipart/form-data">
                <label>Sayfa başlığı: </label><input type="text" name="kartonbardak_title" class="form-control" placeholder="Sayfa Başlığı" />

                <label>Resim: </label><input type="file" name="resim1" class="form-control" placeholder=" Küçük Resim URL" />

                <label>Sayfa İçerik: </label><textarea name="kartonbardak_text"></textarea>
                <script>
