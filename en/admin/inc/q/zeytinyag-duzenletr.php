...
if ($_POST) {
    ...
    if (empty($_FILES["resim1"]['name'])) {
        ...
        $insert = $db->exec("UPDATE  zeytinyagtr SET zeytinyag_p=0, zeytinyag_text='{$Konala_text}' WHERE zeytinyagid={$id}");
        ...
        if (move_uploaded_file($_FILES["$posted"]['tmp_name'], "resimler/" . $yeniad)) {
            $insert = $db->exec("UPDATE  zeytinyagtr SET  zeytinyag_url='{$hedef1}',zeytinyag_p=0, zeytinyag_text='{$Konala_text}' WHERE zeytinyagid={$id}");
            //$insert->execute();
            echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
            header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=zeytinyagtr");
        } else {
...
$sor = $db->query("SELECT * FROM zeytinyagtr WHERE zeytinyagid={$id}");
$Blog = $sor->fetch(PDO::FETCH_ASSOC);
?>

<section class="section">
    <div class="section-inner">
        <h2 class="heading"><i class="fa fa-plus"></i> Zeytin Yağı Düzenle TR<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=zeytinyagtr"; ?>"><i class="fa fa-file-text-o"></i> içerikleri Göster </a></small></h2>
