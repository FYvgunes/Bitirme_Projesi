...
<h2 class="heading"><i class="fa fa-file-text-o"></i>Köpük bardak TR<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=kopukbardaksayfa-ekletr"; ?>"><i class="fa fa-plus"></i>
...
if ($_POST) {
    ...
    if (empty($_FILES["resim1"]['name'])) {
        ...
        $insert = $db->exec("UPDATE  kopukbardaksayfatr SET kopukbardak_text='{$sayfa_text}',kopukbardak_title='{$sayfa_title}' ");
        echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";

        header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=kopukbardaksayfatr");
        ...
        if (move_uploaded_file($_FILES["$posted"]['tmp_name'], "resimler/" . $yeniad)) {
            $insert = $db->exec("UPDATE  kopukbardaksayfatr SET  kopukbardak_url	='{$hedef1}',kopukbardak_title='{$sayfa_title}',kopukbardak_text='{$sayfa_text}' ");
            //$insert->execute();
            echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
            header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=kopukbardaksayfatr");
        } else {
...
$setting = $db->query("SELECT * FROM kopukbardaksayfatr ")->fetch(PDO::FETCH_ASSOC);
?>
