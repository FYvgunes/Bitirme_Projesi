...
<h2 class="heading"><i class="fa fa-file-text-o"></i>Salca Ve Soslar TR<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=salcalarsayfa-ekletr"; ?>"><i class="fa fa-plus"></i>
...
if ($_POST) {
    ...
    if (empty($_FILES["resim1"]['name'])) {
        ...
        $insert = $db->exec("UPDATE  salcalarsayfatr SET salcalar_text='{$sayfa_text}',salcalar_title='{$sayfa_title}' ");
        echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";

        header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=salcalarsayfatr");
        ...
        if (move_uploaded_file($_FILES["$posted"]['tmp_name'], "resimler/" . $yeniad)) {
            $insert = $db->exec("UPDATE  salcalarsayfatr SET  salcalar_url	='{$hedef1}',salcalar_title='{$sayfa_title}',zeytinyag_text='{$sayfa_text}' ");
            //$insert->execute();
            echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
            header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=salcalarsayfatr");
        } else {
...
$setting = $db->query("SELECT * FROM salcalarsayfatr ")->fetch(PDO::FETCH_ASSOC);
?>
