...
<section class="section">
    <div class="section-inner">
        <h2 class="heading"><i class="fa fa-file-text-o"></i> Kanola Yağı DE<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=Kanola-yagi-eklede"; ?>"><i class="fa fa-plus"></i> İçerik Ekle</a></small><br> <small><a href="<?php echo URL . "/en/admin/index.php?do=kanolayagisayfade"; ?>"><i class="fa fa-plus"></i> Sayfa Yapısını Değiştir</a></small></h2>
        <div class="item row">
            <?php
            $sorgu = $db->query("SELECT kanolayagiid FROM kanolayagide WHERE kanolayagi_p=0 ", PDO::FETCH_ASSOC);
            ...
            $row = $db->query("SELECT * FROM kanolayagide WHERE kanolayagi_p=0  ORDER BY kanolayagiid DESC  LIMIT $baslangic, $limit", PDO::FETCH_ASSOC);
            foreach ($row as $content) {
            ?>
                <div class="list-group-item list-trend">
                    <div class="clearfix content-heading">
                        <img class="pull-left img-trend" src="<?php echo $content['kanolayagi_url']; ?>" />
                        <h3 class="txt-trend">Kanola Yağı <?php echo $content['kanolayagiid']; ?> <br /><small><a target="_blank" href="<?php echo URL . "/en/" . $content['content_url']; ?>"><i class="fa fa-eye"></i> Sayfayı Göster</a> | <a href="<?php echo URL . "/en/admin/index.php?do=Kanola-yagi-duzenlede&id=" . $content['kanolayagiid']; ?>"><i class="fa fa-edit"></i> Düzenle</a> | <a onclick="return confirm('İçeriği silmek istediğinizden emin misiniz?');" href="<?php echo URL . "/en/admin/index.php?do=Kanola-yagi-silde&id=" . $content['kanolayagiid']; ?>"><i class="fa fa-trash-o"></i> Sil</a></small></h3>
                    </div>
                </div>
            <?php
            }
