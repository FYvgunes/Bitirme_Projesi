...
<?php
	if($_POST){
		...
		if(empty($blog_title) || empty($blog) || empty($blog_img)){
			...
			$sorgu = $db->prepare("SELECT COUNT(*) FROM blog WHERE Blog_url='{$blog_url}'");
			$sorgu->execute();
			...
			if($say>0){
				...
				$insert = $db->exec("INSERT INTO blog SET  Blog_url='{$blog_url}', Blog_title='{$blog_title}', Blog_img='{$blog_img}', Blog='{$blog}', Blog_p=0,Blog_hit=2, Blog_comments='{$blog_comments}', category_id='{$category_id}' ");
				//$insert->execute();
...
<section class="section">
	...
	<h2 class="heading"><i class="fa fa-plus"></i> Blok Ekle EN<br/><small><a href="<?php echo URL."/en/admin/index.php?do=Blog"; ?>"><i class="fa fa-file-text-o"></i> Blokları Göster </a></small></h2>
