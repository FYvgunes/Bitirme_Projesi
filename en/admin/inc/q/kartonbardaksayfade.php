...
<h2 class="heading"><i class="fa fa-file-text-o"></i>Karton Bardak DE<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=kartonbardaksayfa-eklede"; ?>"><i class="fa fa-plus"></i>
...
if ($_POST) {
    ...
    if (empty($_FILES["resim1"]['name'])) {
        ...
        $insert = $db->exec("UPDATE  kartonbardaksayfade SET kartonbardak_text='{$sayfa_text}',kartonbardak_title='{$sayfa_title}' ");
        echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";

        header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=kartonbardaksayfade");
        ...
        if (move_uploaded_file($_FILES["$posted"]['tmp_name'], "resimler/" . $yeniad)) {
            $insert = $db->exec("UPDATE  kartonbardaksayfade SET  kartonbardak_url	='{$hedef1}',kartonbardak_title='{$sayfa_title}',kartonbardak_text='{$sayfa_text}' ");
            //$insert->execute();
            echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
            header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=kartonbardaksayfade");
        } else {
...
$setting = $db->query("SELECT * FROM kartonbardaksayfade ")->fetch(PDO::FETCH_ASSOC);
?>
