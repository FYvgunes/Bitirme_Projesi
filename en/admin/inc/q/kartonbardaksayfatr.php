...
<h2 class="heading"><i class="fa fa-file-text-o"></i>Karton Bardak TR<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=kartonbardaksayfa-ekletr"; ?>"><i class="fa fa-plus"></i>
...
if ($_POST) {
    ...
    if (empty($_FILES["resim1"]['name'])) {
        ...
        $insert = $db->exec("UPDATE  kartonbardaksayfatr SET kartonbardak_text='{$sayfa_text}',kartonbardak_title='{$sayfa_title}' ");
        echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";

        header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=kartonbardaksayfatr");
        ...
        if (move_uploaded_file($_FILES["$posted"]['tmp_name'], "resimler/" . $yeniad)) {
            $insert = $db->exec("UPDATE  kartonbardaksayfatr SET  kartonbardak_url	='{$hedef1}',kartonbardak_title='{$sayfa_title}',kartonbardak_text='{$sayfa_text}' ");
            //$insert->execute();
            echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
            header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=kartonbardaksayfatr");
        } else {
...
$setting = $db->query("SELECT * FROM kartonbardaksayfatr ")->fetch(PDO::FETCH_ASSOC);
?>
