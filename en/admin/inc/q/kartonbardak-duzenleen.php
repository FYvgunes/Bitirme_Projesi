...
if ($_POST) {
    ...
    if (empty($_FILES["resim1"]['name'])) {
        ...
        $insert = $db->exec("UPDATE  kartonbardaken SET kartonbardak_p=0, kartonbardak_title='{$Konala_text}' WHERE kartonbardak_id={$id}");
        ...
        if (move_uploaded_file($_FILES["$posted"]['tmp_name'], "resimler/" . $yeniad)) {
            $insert = $db->exec("UPDATE  kartonbardaken SET  kartonbardak_url='{$hedef1}',kartonbardak_p=0, kartonbardak_title='{$Konala_text}' WHERE kartonbardak_id={$id}");
            //$insert->execute();
            echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
            header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=kartonbardaken");
        } else {
...
$sor = $db->query("SELECT * FROM kartonbardaken WHERE kartonbardak_id={$id}");
$Blog = $sor->fetch(PDO::FETCH_ASSOC);
?>

<section class="section">
    <div class="section-inner">
        <h2 class="heading"><i class="fa fa-plus"></i> Karton Bardak Düzenle EN<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=kartonbardaken"; ?>"><i class="fa fa-file-text-o"></i> içerikleri Göster </a></small></h2>
