...
if ($_POST) {
    ...
    if (empty($_FILES["resim1"]['name'])) {
        ...
        $insert = $db->exec("UPDATE  donukdeniztr SET donukdeniz_p=0, donukdeniz_text='{$Konala_text}' WHERE donukdeniz_id={$id}");
        ...
        if (move_uploaded_file($_FILES["$posted"]['tmp_name'], "resimler/" . $yeniad)) {
            $insert = $db->exec("UPDATE  donukdeniztr SET  donukdeniz_url='{$hedef1}',donukdeniz_p=0, donukdeniz_text='{$Konala_text}' WHERE donukdeniz_id={$id}");
            //$insert->execute();
            echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
            header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=donukdeniztr");
        } else {
...
$sor = $db->query("SELECT * FROM donukdeniztr WHERE donukdeniz_id={$id}");
$Blog = $sor->fetch(PDO::FETCH_ASSOC);
?>

<section class="section">
    <div class="section-inner">
        <h2 class="heading"><i class="fa fa-plus"></i> Donuk Deniz Ürünleri Düzenle TR<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=donukdeniztr"; ?>"><i class="fa fa-file-text-o"></i> içerikleri Göster </a></small></h2>
