...
$id = g('id');
	...
	if($_POST){
		...
		if(empty($category_title)){
			...
			$sorgu = $db->prepare("SELECT COUNT(*) FROM categoriestr WHERE category_url='{$category_url}' AND category_id != {$id}");
			$sorgu->execute();
			...
			if($say>0){
				...
				$insert = $db->prepare("UPDATE categoriestr SET category_title='{$category_title}', category_url='{$category_url}', category_menu='{$category_menu}', category_onoff=1 WHERE category_id={$id}");
				$insert->execute();
...
<?php
	if(empty($id) || !is_numeric($id)){ header("Location: ".URL."/en/admin/index.php?do=kategoritr"); exit; }else{
		...
		if($icerik_varmi>0){ null; }else{
			header("Location: ".URL."/en/admin/index.php?do=kategoritr"); exit;
		}
	}
?>
...
<section class="section">
	<div class="section-inner">
		<h2 class="heading"><i class="fa fa-edit"></i> Kategori Düzenle TR<br/><small><a href="<?php echo URL."/en/admin/index.php?do=kategoritr"; ?>"><i class="fa fa-folder"></i> Kategorileri Göster</a></small></h2>
		<div class="item row">
