...
<h2 class="heading"><i class="fa fa-file-text-o"></i>Salcalar Ve Soslar DE<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=salcalarsayfa-eklede"; ?>"><i class="fa fa-plus"></i>
...
if ($_POST) {
    ...
    $sayfa_title = p('salcalar_title');
    $sayfa_text = p('zeytinyag_text');
    $posted = "resim1";

    if (empty($_FILES["resim1"]['name'])) {

        $insert = $db->exec("UPDATE  salcalarsayfade SET salcalar_text='{$sayfa_text}',salcalar_title='{$sayfa_title}' ");
        echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";

        header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=salcalarsayfade");
        ...
        if (move_uploaded_file($_FILES["$posted"]['tmp_name'], "resimler/" . $yeniad)) {
            $insert = $db->exec("UPDATE  salcalarsayfade SET  salcalar_url	='{$hedef1}',salcalar_title='{$sayfa_title}',zeytinyag_text='{$sayfa_text}' ");
            //$insert->execute();
            echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
            header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=salcalarsayfade");
        } else {
...
$setting = $db->query("SELECT * FROM salcalarsayfade ")->fetch(PDO::FETCH_ASSOC);
?>
    ...
    <div class="section-inner">
        ...
        <div class="item row">
            <form action="" method="POST" enctype="multipart/form-data">
                <img src="<?php echo $setting['salcalar_url']; ?>" style="width: 250px;  height: 200px; object-fit: cover;  " alt=""> <br>
                <label>Sayfa Başlıgı: </label><input type="text" name="salcalar_title" class="form-control" placeholder="Sayfa Başlığı" value="<?php echo $setting['salcalar_title']; ?>" />
