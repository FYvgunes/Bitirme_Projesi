...
if ($_POST) {
    ...
    if (empty($_FILES["resim1"]['name'])) {
        ...
        $insert = $db->exec("UPDATE  donuksebzede SET donuksebze_p=0, donuksebze_text='{$Konala_text}' WHERE donuksebze_id={$id}");
        ...
        if (move_uploaded_file($_FILES["$posted"]['tmp_name'], "resimler/" . $yeniad)) {
            $insert = $db->exec("UPDATE  donuksebzede SET  donuksebze_url='{$hedef1}',donuksebze_p=0, donuksebze_text='{$Konala_text}' WHERE donuksebze_id={$id}");
            //$insert->execute();
            echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
            header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=donuksebzede");
        } else {
...
$sor = $db->query("SELECT * FROM donuksebzede WHERE donuksebze_id={$id}");
$Blog = $sor->fetch(PDO::FETCH_ASSOC);
?>

<section class="section">
    <div class="section-inner">
        <h2 class="heading"><i class="fa fa-plus"></i> Donuk Sebze Ürünleri Düzenle DE<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=donuksebzede"; ?>"><i class="fa fa-file-text-o"></i> içerikleri Göster </a></small></h2>
