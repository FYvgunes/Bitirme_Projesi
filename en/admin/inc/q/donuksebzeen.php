...
<section class="section">
    <div class="section-inner">
        <h2 class="heading"><i class="fa fa-file-text-o"></i> Donuk Sebze Ürünleri EN<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=donuksebze-ekleen"; ?>"><i class="fa fa-plus"></i> İçerik Ekle</a></small><br> <small><a href="<?php echo URL . "/en/admin/index.php?do=donuksebzesayfaen"; ?>"><i class="fa fa-plus"></i> Sayfa Yapısını Değiştir</a></small><br> <small><a href="<?php echo URL . "/en/admin/index.php?do=donuksebzesayfatr"; ?>"><i class="fa fa-plus"></i> Sayfa Yapısını Değiştir</a></small></h2>
        <div class="item row">
            <?php
            $sorgu = $db->query("SELECT donuksebze_id FROM donuksebzeen WHERE donuksebze_p=0 ", PDO::FETCH_ASSOC);
            ...
            $row = $db->query("SELECT * FROM donuksebzeen WHERE donuksebze_p=0  ORDER BY donuksebze_id DESC  LIMIT $baslangic, $limit", PDO::FETCH_ASSOC);
            foreach ($row as $content) {
            ?>
                <div class="list-group-item list-trend">
                    <div class="clearfix content-heading">
                        <img class="pull-left img-trend" src="<?php echo $content['donuksebze_url']; ?>" />
                        <h3 class="txt-trend">Donuk Deniz Ürünleri <?php echo $content['donuksebze_id']; ?> <br /><small><a target="_blank" href="<?php echo URL . "/en/" . $content['donuksebze_url']; ?>"><i class="fa fa-eye"></i> Sayfayı Göster</a> | <a href="<?php echo URL . "/en/admin/index.php?do=donuksebze-duzenleen&id=" . $content['donuksebze_id']; ?>"><i class="fa fa-edit"></i> Düzenle</a> | <a onclick="return confirm('İçeriği silmek istediğinizden emin misiniz?');" href="<?php echo URL . "/en/admin/index.php?do=donuksebze-silen&id=" . $content['donuksebze_id']; ?>"><i class="fa fa-trash-o"></i> Sil</a></small></h3>
                    </div>
                </div>
            <?php
            }
