...
<?php
	...
	if(empty($id) || !is_numeric($id)){ header("Location: ".URL."/en/admin/index.php?do=Blog"); exit; }else{
		$sor = $db->query("SELECT * FROM blog WHERE Blog_id={$id} AND Blog_p=0");
		$Blog = $sor->fetch(PDO::FETCH_ASSOC);
		$Blog_varmi = $sor->rowCount();
		if($Blog_varmi>0){ null; }else{
			header("Location: ".URL."/en/admin/index.php?do=Blog"); exit;
		}
	}
	...
	if($_POST){
		...
		if(empty($blog_title) || empty($blog) || empty($blog_img)){
			...
			$sorgu = $db->prepare("SELECT COUNT(*) FROM blog WHERE Blog_url='{$blog_url}' AND Blog_id != {$id}");
			$sorgu->execute();
			...
			if($say>0){
				...
				$insert = $db->prepare("UPDATE blog SET Blog_title='{$blog_title}', Blog_img='{$blog_img}', Blog_comments='{$blog_comments}', category_id='{$category_id}', Blog='{$blog}', Blog_p=0, Blog_url='{$blog_url}' WHERE Blog_id={$id}");
				$insert->execute();
				echo "<p class='alert alert-success'>Blog başarıyla düzenlendi. Lütfen Bekleyiniz...</p>";
				header("Refresh: 2; url=".URL."/en/admin/index.php?do=Blog");
			}
...
<section class="section">
	<div class="section-inner">
		<h2 class="heading"><i class="fa fa-edit"></i> Blog Düzenle EN<br/><small><a href="<?php echo URL."/en/admin/index.php?do=Blog"; ?>"><i class="fa fa-file-text-o"></i> Blog Göster</a></small></h2>
		<div class="item row">
