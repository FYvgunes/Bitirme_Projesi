...
$id = g('id');
if (empty($id)) {
    header("Location: " . URL . "/en/admin/index.php?do=salcalarde");
    exit;
} else {
    $sor = $db->query("SELECT * FROM salcalarde WHERE salcalarid={$id} AND salcalar_p=0");
...
if ($_POST) {
    ...
    if (empty($_FILES["resim1"]['name'])) {
        ...
        $insert = $db->exec("UPDATE  salcalarde SET salcalar_p=0, salcalar_title='{$Konala_text}' WHERE salcalarid={$id}");
        echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
        header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=salcalarde");
        ...
        if (move_uploaded_file($_FILES["$posted"]['tmp_name'], "resimler/" . $yeniad)) {
            $insert = $db->exec("UPDATE  salcalarde SET  salcalar_url='{$hedef1}',salcalar_p=0, salcalar_title='{$Konala_text}' WHERE salcalarid={$id}");
            //$insert->execute();
            echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
            header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=salcalarde");
        } else {
...
$sor = $db->query("SELECT * FROM salcalarde WHERE salcalarid={$id}");
$Blog = $sor->fetch(PDO::FETCH_ASSOC);
?>

<section class="section">
    <div class="section-inner">
        <h2 class="heading"><i class="fa fa-plus"></i> Zeytin Yağı Düzenle DE<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=salcalarde"; ?>"><i class="fa fa-file-text-o"></i> içerikleri Göster </a></small></h2>
