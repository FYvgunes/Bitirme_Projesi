...
<h2 class="heading"><i class="fa fa-file-text-o"></i>Peynir DE<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=peynirsayfa-eklede"; ?>"><i class="fa fa-plus"></i>
...
if ($_POST) {
    ...
    if (empty($_FILES["resim1"]['name'])) {
        ...
        $insert = $db->exec("UPDATE  peynirsayfade SET peynir_text='{$sayfa_text}',peynir_title='{$sayfa_title}' ");
        echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";

        header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=peynirsayfade");
        ...
        if (move_uploaded_file($_FILES["$posted"]['tmp_name'], "resimler/" . $yeniad)) {
            $insert = $db->exec("UPDATE  peynirsayfade SET  peynir_url	='{$hedef1}',peynir_title='{$sayfa_title}',peynir_text='{$sayfa_text}' ");
            //$insert->execute();
            echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
            header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=peynirsayfade");
        } else {
...
$setting = $db->query("SELECT * FROM peynirsayfade ")->fetch(PDO::FETCH_ASSOC);
?>
