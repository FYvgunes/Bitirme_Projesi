...
<h2 class="heading"><i class="fa fa-file-text-o"></i>AYçiçek yağı TR<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=ayciceksayfa-ekletr"; ?>"><i class="fa fa-plus"></i>
...
if ($_POST) {
    ...
    if (empty($_FILES["resim1"]['name'])) {
        ...
        $insert = $db->exec("UPDATE  ayciceksayfatr SET aycicegiyag_text='{$sayfa_text}',aycicegiyag_title='{$sayfa_title}' ");
        echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";

        header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=ayciceksayfatr");
        ...
        if (move_uploaded_file($_FILES["$posted"]['tmp_name'], "resimler/" . $yeniad)) {
            $insert = $db->exec("UPDATE  ayciceksayfatr SET  aycicegiyag_url='{$hedef1}',aycicegiyag_title='{$sayfa_title}',aycicegiyag_text='{$sayfa_text}' ");
            //$insert->execute();
            echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
            header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=ayciceksayfatr");
        } else {
...
$setting = $db->query("SELECT * FROM ayciceksayfatr ")->fetch(PDO::FETCH_ASSOC);
?>
