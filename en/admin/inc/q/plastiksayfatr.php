...
<h2 class="heading"><i class="fa fa-file-text-o"></i>Plastik Kaplar TR<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=plastitbardaksayfa-ekletr"; ?>"><i class="fa fa-plus"></i>
...
if ($_POST) {
    ...
    if (empty($_FILES["resim1"]['name'])) {
        ...
        $insert = $db->exec("UPDATE  plastikkapsayfatr SET plastikkap_text='{$sayfa_text}',plastikkap_title='{$sayfa_title}' ");
        echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";

        header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=plastiksayfatr");
        ...
        if (move_uploaded_file($_FILES["$posted"]['tmp_name'], "resimler/" . $yeniad)) {
            $insert = $db->exec("UPDATE  plastikkapsayfatr SET  plastikkap_url	='{$hedef1}',plastikkap_title='{$sayfa_title}',plastikkap_text='{$sayfa_text}' ");
            //$insert->execute();
            echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
            header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=plastiksayfatr");
        } else {
...
$setting = $db->query("SELECT * FROM plastikkapsayfatr ")->fetch(PDO::FETCH_ASSOC);
?>
