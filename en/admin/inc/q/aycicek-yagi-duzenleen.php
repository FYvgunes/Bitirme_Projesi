...
$id = g('id');
if (empty($id)) {
    header("Location: " . URL . "/en/admin/index.php?do=aycicek-yagien");
    exit;
} else {
    $sor = $db->query("SELECT * FROM aycicegiyagen WHERE aycicegiyagid={$id} AND aycicegiyag_p=0");
...
if ($_POST) {
    ...
    if (empty($_FILES["resim1"]['name'])) {
        ...
        $insert = $db->exec("UPDATE  aycicegiyagen SET aycicegiyag_p=0, aycicegiyag_text='{$Konala_text}' WHERE aycicegiyagid={$id}");
        ...
        if (move_uploaded_file($_FILES["$posted"]['tmp_name'], "resimler/" . $yeniad)) {
            $insert = $db->exec("UPDATE  aycicegiyagen SET  aycicegiyag_url='{$hedef1}',aycicegiyag_p=0, aycicegiyag_text='{$Konala_text}' WHERE aycicegiyagid={$id}");
            //$insert->execute();
            echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
            header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=aycicek-yagien");
        } else {
...
$sor = $db->query("SELECT * FROM aycicegiyagen WHERE aycicegiyagid={$id}");
$Blog = $sor->fetch(PDO::FETCH_ASSOC);
?>

<section class="section">
    <div class="section-inner">
        <h2 class="heading"><i class="fa fa-plus"></i> Ayçicek Düzenle EN<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=aycicek-yagien"; ?>"><i class="fa fa-file-text-o"></i> Blokları Göster </a></small></h2>
