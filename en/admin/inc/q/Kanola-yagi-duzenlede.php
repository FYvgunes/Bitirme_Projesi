...
$id = g('id');
if (empty($id)) {
    header("Location: " . URL . "/en/admin/index.php?do=Kanolo-yagde");
    exit;
} else {
    $sor = $db->query("SELECT * FROM kanolayagide WHERE kanolayagiid={$id} AND kanolayagi_p=0");
    $Blog = $sor->fetch(PDO::FETCH_ASSOC);
    $Blog_varmi = $sor->rowCount();
    if ($Blog_varmi > 0) {
        null;
    } else {
        header("Location: " . URL . "/en/admin/index.php?do=Kanola-yagide");
        exit;
    }
}
...
if ($_POST) {
    ...
    if (empty($_FILES["resim1"]['name'])) {
        ...
        $insert = $db->exec("UPDATE  kanolayagide SET kanolayagi_p=0, kanolayagi_text='{$Konala_text}' WHERE kanolayagiid={$id}");
        echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
        header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=Kanola-yagide");
        ...
        if (move_uploaded_file($_FILES["$posted"]['tmp_name'], "resimler/" . $yeniad)) {
            $insert = $db->exec("UPDATE  kanolayagide SET  kanolayagi_url='{$hedef1}',kanolayagi_p=0, kanolayagi_text='{$Konala_text}' WHERE kanolayagiid={$id}");
            //$insert->execute();
            echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
            header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=Kanola-yagide");
        } else {
...
$sor = $db->query("SELECT * FROM kanolayagide WHERE kanolayagiid={$id}");
$Blog = $sor->fetch(PDO::FETCH_ASSOC);
?>

<section class="section">
    <div class="section-inner">
        <h2 class="heading"><i class="fa fa-plus"></i> Kanola Yağı Düzenle DE<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=anola-yagide"; ?>"><i class="fa fa-file-text-o"></i> İçerikleri Göster </a></small></h2>
