...
<h2 class="heading"><i class="fa fa-file-text-o"></i>Peçeteler DE<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=pecetelersayfa-eklede"; ?>"><i class="fa fa-plus"></i>
...
if ($_POST) {
    ...
    if (empty($_FILES["resim1"]['name'])) {
        ...
        $insert = $db->exec("UPDATE  pecetelersayfade SET peceteler_text='{$sayfa_text}',peceteler_title='{$sayfa_title}' ");
        echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";

        header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=pecetelersayfade");
        ...
        if (move_uploaded_file($_FILES["$posted"]['tmp_name'], "resimler/" . $yeniad)) {
            $insert = $db->exec("UPDATE  pecetelersayfade SET  peceteler_url	='{$hedef1}',peceteler_title='{$sayfa_title}',peceteler_text='{$sayfa_text}' ");
            //$insert->execute();
            echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
            header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=pecetelersayfade");
        } else {
...
$setting = $db->query("SELECT * FROM pecetelersayfade ")->fetch(PDO::FETCH_ASSOC);
?>
