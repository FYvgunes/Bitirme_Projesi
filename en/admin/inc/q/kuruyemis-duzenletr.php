...
$id = g('id');
if (empty($id)) {
    header("Location: " . URL . "/en/admin/index.php?do=kuruyemistr");
    exit;
} else {
    $sor = $db->query("SELECT * FROM kuruyemistr WHERE kuruyemis_id={$id} AND kuruyemis_p=0");
...
if ($_POST) {
    ...
    if (empty($_FILES["resim1"]['name'])) {
        ...
        $insert = $db->exec("UPDATE  kuruyemistr SET kuruyemis_p=0, kuruyemis_text='{$Konala_text}' WHERE kuruyemis_id={$id}");
        echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
        header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=kuruyemistr");
        ...
        if (move_uploaded_file($_FILES["$posted"]['tmp_name'], "resimler/" . $yeniad)) {
            $insert = $db->exec("UPDATE  kuruyemistr SET  kuruyemis_url='{$hedef1}',kuruyemis_p=0, kuruyemis_text='{$Konala_text}' WHERE kuruyemis_id={$id}");
            //$insert->execute();
            echo "<p class='alert alert-success'>İçerik başarıyla eklendi. Lütfen Bekleyiniz...</p>";
            header("Refresh: 2; url=" . URL . "/en/admin/index.php?do=kuruyemistr");
        } else {
...
$sor = $db->query("SELECT * FROM kuruyemistr WHERE kuruyemis_id={$id}");
$Blog = $sor->fetch(PDO::FETCH_ASSOC);
?>

<section class="section">
    <div class="section-inner">
        <h2 class="heading"><i class="fa fa-plus"></i> Kuruyemiş Düzenle TR<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=kuruyemistr"; ?>"><i class="fa fa-file-text-o"></i> içerikleri Göster </a></small></h2>
