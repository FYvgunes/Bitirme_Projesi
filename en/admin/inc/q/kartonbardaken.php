...
<section class="section">
    <div class="section-inner">
        <h2 class="heading"><i class="fa fa-file-text-o"></i> Karton Bardak EN<br /><small><a href="<?php echo URL . "/en/admin/index.php?do=kartonbardak-ekleen"; ?>"><i class="fa fa-plus"></i> İçerik Ekle</a></small><br> <small><a href="<?php echo URL . "/en/admin/index.php?do=kartonbardaksayfaen"; ?>"><i class="fa fa-plus"></i> Sayfa Yapısını Değiştir</a></small></h2>
        <div class="item row">
            <?php
            $sorgu = $db->query("SELECT kartonbardak_id FROM kartonbardaken WHERE kartonbardak_p=0 ", PDO::FETCH_ASSOC);
            ...
            $row = $db->query("SELECT * FROM kartonbardaken WHERE kartonbardak_p=0  ORDER BY kartonbardak_id DESC  LIMIT $baslangic, $limit", PDO::FETCH_ASSOC);
            foreach ($row as $content) {
            ?>
                <div class="list-group-item list-trend">
                    <div class="clearfix content-heading">
                        <img class="pull-left img-trend" src="<?php echo $content['kartonbardak_url']; ?>" />
                        <h3 class="txt-trend">Karton Bardaklar <?php echo $content['kartonbardak_id']; ?> <br /><small><a target="_blank" href="<?php echo URL . "/en/" . $content['kartonbardak_url']; ?>"><i class="fa fa-eye"></i> Sayfayı Göster</a> | <a href="<?php echo URL . "/en/admin/index.php?do=kartonbardak-duzenleen&id=" . $content['kartonbardak_id']; ?>"><i class="fa fa-edit"></i> Düzenle</a> | <a onclick="return confirm('İçeriği silmek istediğinizden emin misiniz?');" href="<?php echo URL . "/en/admin/index.php?do=kartonbardak-silen&id=" . $content['kartonbardak_id']; ?>"><i class="fa fa-trash-o"></i> Sil</a></small></h3>
                    </div>
                </div>
            <?php
            }
